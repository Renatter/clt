<template>
  <div class="container">
    <h1 class="text-[20px] font-bold text-[orange]">   {{ $t('message.order.title') }}</h1>
    <div
      v-for="i in item"
      :key="i.id"
      @click="openModal(i)"
  ...
  <div v-if="showModal" class="modal">
    <div class="modal-content">
      <h2>
        {{ $t('message.order.name') }}: <span class="text-[red]">{{ selectedItem.name }}</span>
      </h2>
      <p>
        {{ $t('message.order.city') }}: <span class="text-[red]">{{ selectedItem.city }}</span>
      </p>
      <div class="flex">
        <p>
          Адрес:
      ...
      <div class="flex" v-for="i in selectedItem.cart">
        ...
        <div class="ml-[20px]">
          ...
          <div class="text-[20px]">
            {{ $t('message.order.size') }}: <span class="text-[red]">{{ i.size }}</span>
          </div>
          <div class="text-[20px]">
            {{ $t('message.order.price') }}: <span class="text-[red]"> {{ i.price }} тг</span>
          </div>
        </div>
      </div>
      <div class="pt-[15px]">
